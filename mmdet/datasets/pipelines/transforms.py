...
@PIPELINES.register_module()
class MixUp(object):
    def __init__(self, p=0.5, alpha=1):
        self.alpha = alpha
        self.p = p
        self.img2 = None
        self.boxes2 = None
    ...
    def __call__(self, results):
        ...
        if random.random() < self.p and self.img2 is not None and img1.shape[1] == self.img2.shape[1]:
            ...
            mixup_image[:img1.shape[0], :img1.shape[1], :] = img1.astype('float32') * lam
            mixup_image[:self.img2.shape[0], :self.img2.shape[1], :] += self.img2.astype('float32') * (1. - lam)
